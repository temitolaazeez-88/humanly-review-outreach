/* HumanlyReview shared frontend helpers — v8.0.0 (exact-plan access, affiliates, trust UX & rollback). */
...
const HR_STORAGE = Object.freeze({
  ...
  PUBLIC_CONFIG_CACHE: 'hr_public_config_cache_v5',
  AFFILIATE_REF: 'hr_affiliate_ref_v1',
  AFFILIATE_CLICK: 'hr_affiliate_click_v1',
  AFFILIATE_CLIENT: 'hr_affiliate_client_v1'
});
...
function hrBuildReviewMeta(existing={}){
  const now=Date.now();
  return {version:7,startedAt:Number(existing.startedAt)||now,updatedAt:now,expiresAt:Number(existing.expiresAt)||now+HR_CONFIG.REVIEW_RETENTION_MS,sessionToken:existing.sessionToken||'',pendingSessionToken:existing.pendingSessionToken||'',email:existing.email||'',orderRef:existing.orderRef||'',customTicketRef:existing.customTicketRef||'',accessGrantToken:existing.accessGrantToken||'',accessCodeId:existing.accessCodeId||'',accessCodeLabel:existing.accessCodeLabel||'',marketingConsent:existing.marketingConsent===true,status:existing.status||'draft',resumeStep:existing.resumeStep||'step-email',briefChoice:existing.briefChoice||'',instructionText:existing.instructionText||'',workFilesMeta:Array.isArray(existing.workFilesMeta)?existing.workFilesMeta:[],instructionFilesMeta:Array.isArray(existing.instructionFilesMeta)?existing.instructionFilesMeta:[]};
}
    ...
    await new Promise((resolve,reject)=>{
      ...
      allReq.onsuccess=()=>{
        try{
          ...
          realFiles.forEach((f,i)=>store.put({kind,index:i,name:f.name,size:Number(f.size||0),type:f.type||'application/octet-stream',lastModified:Number(f.lastModified||Date.now()),pageCount:Number(f.__hrPageCount||0)||null,blob:f}));
...
async function hrRestoreDraftFiles(kind){
  try{
    ...
    for(const r of rows){
      ...
      try{const f=new File([blob],r.name,{type:r.type,lastModified:r.lastModified});if(Number(r.pageCount||0)>0)f.__hrPageCount=Number(r.pageCount);restored.push(f);}catch(_){}
    }
    return restored;
  }catch(e){
...
function hrMakeMissingDraftFile(item){
  return {name:String(item.name||'Saved file'),size:Number(item.size)>0?Number(item.size):null,type:item.type||'application/octet-stream',lastModified:Number(item.lastModified||Date.now()),__hrPageCount:Number(item.pageCount||0)||null,__draftMissing:true};
}
function hrMergeRestoredDraftFiles(kind,metaItems,actualFiles){
  ...
  return metaItems.map(item=>{
    ...
    if(idx>=0){used.add(idx);if(Number(item.pageCount||0)>0&&!Number(actual[idx].__hrPageCount||0))actual[idx].__hrPageCount=Number(item.pageCount);return actual[idx];}
...
async function hrRestoreDraftIntoState(){
  ...
  hrState.accessGrantToken=meta?.accessGrantToken||hrState.accessGrantToken||'';hrState.accessCodeId=meta?.accessCodeId||hrState.accessCodeId||'';hrState.accessCodeLabel=meta?.accessCodeLabel||hrState.accessCodeLabel||'';hrState.marketingConsent=meta?.marketingConsent===true;
...
const hrState={
  ...
  accessGrantToken:_hrMeta?.accessGrantToken||'',accessCodeId:_hrMeta?.accessCodeId||'',accessCodeLabel:_hrMeta?.accessCodeLabel||'',marketingConsent:_hrMeta?.marketingConsent===true
};
...
const HR_READ_ACTIONS=new Set(['health','getPublicConfig','checkSession','matchPlan','getOrderStatus','getCustomRequestStatus','getDelivery','getPaidOrderRecovery','getDownloadChunk','getAffiliateDashboard']);
...
function hrNormalizeAffiliateCode(v){return String(v||'').trim().toUpperCase().replace(/[^A-Z0-9_-]/g,'').slice(0,40);}
function hrAffiliateClientId(){
  let id='';try{id=localStorage.getItem(HR_STORAGE.AFFILIATE_CLIENT)||'';}catch(_){}
  if(id)return id;
  try{id=(crypto&&crypto.randomUUID)?crypto.randomUUID():'c-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2);}catch(_){id='c-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2);}
  try{localStorage.setItem(HR_STORAGE.AFFILIATE_CLIENT,id);}catch(_){}return id;
}
function hrGetAffiliateRef(){
  const row=hrReadJson(HR_STORAGE.AFFILIATE_REF,null);if(!row||!row.code)return null;
  if(Number(row.expiresAt||0)&&Number(row.expiresAt)<Date.now()){try{localStorage.removeItem(HR_STORAGE.AFFILIATE_REF);}catch(_){}return null;}
  return row;
}
function hrGetAffiliateCode(){const row=hrGetAffiliateRef();return row?String(row.code||''):'';}
async function hrCaptureAffiliateRef(){
  let q='';try{q=hrNormalizeAffiliateCode(hrGetQuery().get('ref')||'');}catch(_){}
  if(!q)return;
  const defaultDays=30,existing=hrGetAffiliateRef();
  hrWriteJson(HR_STORAGE.AFFILIATE_REF,{code:q,capturedAt:Date.now(),expiresAt:Date.now()+defaultDays*86400000});
  const marker=hrReadJson(HR_STORAGE.AFFILIATE_CLICK,{}),key=q+'|'+new Date().toISOString().slice(0,10);if(marker&&marker.key===key)return;
  try{
    const out=await hrApi('trackAffiliateClick',{code:q,clientId:hrAffiliateClientId()},{retries:0});
    const days=Math.max(1,Number(out&&out.attributionDays||defaultDays));hrWriteJson(HR_STORAGE.AFFILIATE_REF,{code:q,capturedAt:Date.now(),expiresAt:Date.now()+days*86400000});hrWriteJson(HR_STORAGE.AFFILIATE_CLICK,{key:key,at:Date.now()});
  }catch(_){/* Referral attribution is non-blocking; checkout validates again server-side. */}
}
function hrBootUx(){hrEnhanceAll();hrInstallEnterKeys();hrInitDisplayControls();hrInitMobileMenu();hrCaptureAffiliateRef();}
