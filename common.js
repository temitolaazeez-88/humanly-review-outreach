...
function hrBuildReviewMeta(existing={}){
  const now=Date.now();
  return {version:3,startedAt:Number(existing.startedAt)||now,updatedAt:now,expiresAt:Number(existing.expiresAt)||now+HR_CONFIG.REVIEW_RETENTION_MS,sessionToken:existing.sessionToken||'',email:existing.email||'',orderRef:existing.orderRef||'',customTicketRef:existing.customTicketRef||'',status:existing.status||'draft',resumeStep:existing.resumeStep||'step-email',briefChoice:existing.briefChoice||'',instructionText:existing.instructionText||''};
}
...
async function hrRestoreDraftIntoState(){
  ...
  hrState.sessionToken=meta.sessionToken||hrState.sessionToken||'';
  hrState.email=meta.email||hrState.email||'';
...
async function hrClearCurrentReviewStorage(){
  Object.values(HR_STORAGE).filter(k=>k!==HR_STORAGE.DRAFT_DB&&k!==HR_STORAGE.DRAFT_STORE).forEach(key=>localStorage.removeItem(key));
  await hrClearDraftFiles();
}
function hrHasActiveReview(){const meta=hrGetReviewMeta();if(!meta)return false;return !!(meta.sessionToken||meta.orderRef||meta.customTicketRef||String(meta.resumeStep||'step-email')!=='step-email');}
...
(function hrInstallCustomerFlowGuard(){
  ...
  function completelyResetBrowserReview(showFresh=true){
    ...
    const ok=window.confirm(hasSaved
      ? 'Start a new review? Your unfinished review will be cleared from this device. Any existing paid order will NOT be deleted or refunded.'
      : 'Start a new review?');
    if(!ok)return false;
