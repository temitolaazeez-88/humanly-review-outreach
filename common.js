/* Humanly Review shared frontend helpers — customer flow v6.3.1. */
...
async function hrCheckVerifiedSession(email=hrState.email,token=hrState.sessionToken){if(!email||!token)return false;try{const r=await hrApi('checkSession',{sessionToken:token,email},{noRetry:true});if(r?.data?.valid){hrSetVerifiedSession(token,email,r.data.expiresAt||'');hrState.sessionToken=token;hrState.email=email;try{localStorage.setItem(HR_STORAGE.LEGACY_SESSION,token);localStorage.setItem(HR_STORAGE.LEGACY_EMAIL,email);}catch(_){}return true;}return false;}catch(_){return false;}}
...
async function hrReuseVerifiedIdentity(email){const e=String(email||'').trim().toLowerCase();if(!e)return false;const candidates=[];const stored=hrGetVerifiedSession();if(stored?.sessionToken&&stored?.email&&String(stored.email).toLowerCase()===e)candidates.push(stored);if(hrState.sessionToken&&hrState.email&&String(hrState.email).toLowerCase()===e&&(!stored||String(stored.sessionToken)!==String(hrState.sessionToken)))candidates.push({sessionToken:hrState.sessionToken,email:e,expiresAt:''});for(const v of candidates){if(await hrCheckVerifiedSession(e,v.sessionToken)){hrState.newReviewMode=false;hrState.sessionToken=v.sessionToken;hrState.email=e;return true;}}return false;}
