...
const hrState = {
  ...
  instructionText: '',
  currentOrder: null,
  customTicketRef: localStorage.getItem('hr_custom_ticket') || ''
};
...
function hrSaveCustomTicket(ticketRef) { hrState.customTicketRef=ticketRef||''; if(ticketRef)localStorage.setItem('hr_custom_ticket',ticketRef); }
function hrClearCustomTicket() { hrState.customTicketRef=''; localStorage.removeItem('hr_custom_ticket'); }
...
async function hrApi(action,payload={},options={}){
  ...
  const attempts=options.noRetry?1:(Number(HR_CONFIG.MAX_NETWORK_RETRIES)+1);
  let lastErr;
  for(let attempt=0;attempt<attempts;attempt++){
    try{
      const controller=new AbortController();
      const timeout=setTimeout(()=>controller.abort(),options.timeoutMs||45000);
      const res=await fetch(HR_CONFIG.BACKEND_URL,{method:'POST',redirect:'follow',headers:{'Content-Type':'text/plain;charset=utf-8'},body,signal:controller.signal});
      clearTimeout(timeout);
      const text=await res.text();
      let data;
      try{data=JSON.parse(text);}catch(e){throw new Error('The server returned an unreadable response. Please retry.');}
      if(!data.ok)throw new Error(data.error||'The request could not be completed.');
      return data;
    }catch(err){
      lastErr=err;
      if(attempt<attempts-1)await new Promise(r=>setTimeout(r,Math.min(1800,500*(attempt+1))));
    }
  }
