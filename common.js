/* Humanly Review shared frontend helpers — customer flow v6.0.1. */
...
function hrBuildReviewMeta(existing={}){
  const now=Date.now();
  return {version:2,startedAt:Number(existing.startedAt)||now,updatedAt:now,expiresAt:Number(existing.expiresAt)||now+HR_CONFIG.REVIEW_RETENTION_MS,sessionToken:existing.sessionToken||'',email:existing.email||'',orderRef:existing.orderRef||'',customTicketRef:existing.customTicketRef||'',status:existing.status||'draft'};
}
function hrGetReviewMeta(){
  ...
  if(!meta){
    ...
    if(legacyOrder||legacySession||legacyEmail||legacyTicket){
      meta=hrBuildReviewMeta({sessionToken:legacySession,email:legacyEmail,orderRef:legacyOrder,customTicketRef:legacyTicket,status:legacyOrder?'order':legacyTicket?'custom_quote':'draft'});
...
function hrClearCurrentReviewStorage(){Object.values(HR_STORAGE).forEach(key=>localStorage.removeItem(key));}
function hrHasActiveReview(){const meta=hrGetReviewMeta();return !!(meta&&(meta.sessionToken||meta.email||meta.orderRef||meta.customTicketRef));}
const _hrMeta=hrGetReviewMeta();
const hrState={
  sessionToken:_hrMeta?.sessionToken||localStorage.getItem(HR_STORAGE.LEGACY_SESSION)||'',
  email:_hrMeta?.email||localStorage.getItem(HR_STORAGE.LEGACY_EMAIL)||'',
...
function hrSaveSession(token,email){hrState.sessionToken=token||'';hrState.email=email||hrState.email||'';hrPersistReviewMeta({sessionToken:hrState.sessionToken,email:hrState.email,status:hrState.orderRef?'order':'draft'});}
...
function hrStartNewReview(){
  try{Object.values(HR_STORAGE).forEach(key=>localStorage.removeItem(key));}catch(_){ }
  hrState.sessionToken='';hrState.email='';hrState.orderRef='';hrState.customTicketRef='';hrState.paymentTabOpened=false;hrState.selectedFile=null;hrState.selectedWorkFiles=[];hrState.selectedInstructionFiles=[];hrState.instructionText='';hrState.currentOrder=null;hrState.currentOrderPreview=null;hrState.isCustomQuote=false;
}
function hrFinishCurrentReview(){hrStartNewReview();}
...
async function hrApi(action,payload={},options={}){if(!HR_CONFIG.BACKEND_URL||HR_CONFIG.BACKEND_URL.includes('PASTE_'))throw new Error('The Apps Script backend URL has not been configured yet.');const body=JSON.stringify({action,payload,clientVersion:HR_CONFIG.CLIENT_VERSION});const attempts=options.noRetry?1:(Number(HR_CONFIG.MAX_NETWORK_RETRIES)+1);let lastErr;for(let attempt=0;attempt<attempts;attempt++){try{const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),options.timeoutMs||45000);const res=await fetch(HR_CONFIG.BACKEND_URL,{method:'POST',redirect:'follow',headers:{'Content-Type':'text/plain;charset=utf-8'},body,signal:controller.signal});clearTimeout(timeout);const text=await res.text();let data;try{data=JSON.parse(text);}catch(e){throw new Error('The server returned an unreadable response. Please retry.');}if(!data.ok)throw new Error(data.error||'The request could not be completed.');return data;}catch(err){lastErr=err;if(attempt<attempts-1)await new Promise(r=>setTimeout(r,Math.min(1800,500*(attempt+1))));}}throw new Error(lastErr?.name==='AbortError'?'The request took too long. Please try again.':(lastErr?.message||'Network error. Please check your connection and try again.'));}
...
/* Customer-flow UI patch: shared-device reset, continuation and completion cleanup. */
(function hrInstallCustomerFlowGuard(){
  ...
  function completelyResetBrowserReview(showFresh=true){
    const hadOrder=!!hrState.orderRef;
    const ok=window.confirm(hadOrder
      ? 'Start a new review on this device? Your existing order will NOT be deleted or refunded. Only this browser session will be cleared.'
      : 'Start a new review? The current review details on this device will be cleared.');
    if(!ok)return false;
    try{clearInterval(window.paymentPoll);clearInterval(window.countdownTimer);}catch(_){ }
    hrStartNewReview();
    try{if(typeof resetSubmission==='function')resetSubmission();}catch(_){ }
    resetVisibleForm();
    ...
    hrToast('This device is ready for a completely new review.','success');
    updateResumeBox();
    return true;
  }
  ...
  function continueActive(){
    try{
      if(hrState.orderRef){
        ...
        if(typeof resumeExistingOrder==='function')resumeExistingOrder();
      }else if(hrState.sessionToken){
        if(typeof openModal==='function')openModal('step-work');
      }else if(hrState.email){
        if(typeof openModal==='function')openModal('step-email');
  ...
  function updateResumeBox(){
    ...
    box.innerHTML=`<strong>This device has an unfinished review.</strong><div class="small muted" style="margin-top:5px">Continue the previous review or clear this device and start a new one.</div><div class="actions" style="margin-top:12px"><button type="button" class="btn" id="hr-continue-review">Continue Review</button><button type="button" class="btn secondary" id="hr-new-review">Start New Review</button></div>`;
    start.parentElement?.appendChild(box);
    document.getElementById('hr-continue-review')?.addEventListener('click',continueActive);
    document.getElementById('hr-new-review')?.addEventListener('click',()=>completelyResetBrowserReview(false));
  }
  ...
  function addInFlowNewReviewButton(){
    ...
    if(!document.getElementById('hr-modal-new-review')){
      ...
      b.addEventListener('click',()=>completelyResetBrowserReview(false));
    ...
    const observer=new MutationObserver(()=>{
      ...
      if(visible&&!lastState){
        ...
        if(title.includes('received your submission')||title.includes('custom review request is received')||badge.includes('submitted')){
          /* Save nothing sensitive in the browser after the review is complete. */
  ...
  function install(){
    ...
    /* Capture phase prevents the old start button handler from silently reopening an old review. */
    document.addEventListener('click',function(ev){
      ...
      const resume=window.confirm('This device already has an unfinished review.\n\nOK = Continue the previous review.\nCancel = Start a new review and clear this browser session.');
      if(resume)continueActive();
      else completelyResetBrowserReview(true);
    },true);

    /* If the success screen was created after this script ran, add its button then. */
    const modal=document.getElementById('app-modal');
    if(modal){
      new MutationObserver(()=>addInFlowNewReviewButton()).observe(modal,{childList:true,subtree:true});
    }
  }
