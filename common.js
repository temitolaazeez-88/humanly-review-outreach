...
function hrSetVerifiedSession(token,email){if(token&&email)hrWriteJson(HR_STORAGE.VERIFIED_SESSION,{sessionToken:String(token),email:String(email).trim().toLowerCase(),verifiedAt:Date.now()});}
function hrClearVerifiedSession(){try{localStorage.removeItem(HR_STORAGE.VERIFIED_SESSION);}catch(_){} }
async function hrCheckVerifiedSession(email=hrState.email,token=hrState.sessionToken){if(!email||!token)return false;try{const r=await hrApi('checkSession',{sessionToken:token,email});if(r?.data?.valid){hrSetVerifiedSession(token,email);hrState.sessionToken=token;hrState.email=email;return true;}return false;}catch(_){return false;}}
...
async function hrRestoreDraftIntoState(){
  ...
  if(hrState.sessionToken&&hrState.email)hrSetVerifiedSession(hrState.sessionToken,hrState.email);
  try{
...
function hrSaveSession(token,email){hrState.sessionToken=token||'';hrState.email=email||hrState.email||'';if(hrState.sessionToken&&hrState.email)hrSetVerifiedSession(hrState.sessionToken,hrState.email);hrPersistReviewMeta({sessionToken:hrState.sessionToken,email:hrState.email,status:hrState.orderRef?'order':'draft'});}
...
function hrStartNewReview(){
  ...
  hrState.orderRef='';hrState.customTicketRef='';hrState.paymentTabOpened=false;hrState.selectedFile=null;hrState.selectedWorkFiles=[];hrState.selectedInstructionFiles=[];hrState.instructionText='';hrState.currentOrder=null;hrState.currentOrderPreview=null;hrState.isCustomQuote=false;
  hrClearDraftFiles();
}
